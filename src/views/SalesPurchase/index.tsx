...
import Confirm_Alert_Popup from '../../components/popup/ConfirmAlertPopup';
...
export const Sales_Table = ({ type }: SalesPurchaseTableProps) => {
  ...
  const handleConfirmPopup = async () => {
    ...
    if (formData !== initialValue) {
      const endPoint =
        type === 'Sales' ? `/${companyId}/sale` : `/${companyId}/purchase`;
      const endpoint = formData.spId
        ? `${endPoint}/${formData.spId}`
        : `${endPoint}`;
      const method = formData.spId ? 'PUT' : 'POST';
  ...
  const handelFormSubmit = (values: SalesPurchaseFormData) => {
    const mode = values.spId ? 'update' : 'create';
  ...
  const deleteAcc = async (spId: string) => {
    isDelete.current = false;
    togglePopup(false);
    const endPoint =
      type === 'Sales' ? `/${companyId}/sale` : `/${companyId}/purchase`;
    const endpoint = `${endPoint}/${spId}`;
    togglePopup(false);
  ...
  const handleCellEditingStopped = async (e: any) => {
    ...
    const endPoint =
      type === 'Sales' ? `/${companyId}/sale` : `/${companyId}/purchase`;
    const endpoint = `${endPoint}/${data.spId}`;
