...
const CreateVouchers = ({ setView, data }: any) => {
  ...
  const headers = [
    ...
    { name: 'Discount Narration', key: 'discNarration', width: '15%', type: 'input', props: { inputType: 'text', handleChange: (args: any) => { handleInputChange(args); } } },
  ];
...
const initializeGridDataFromData = () => {
  console.log("Initializing Grid Data from Voucher Data...");

  if (data?.voucherGridData) {
    console.log("Processing voucherGridData...");
...
console.log("initialRows---------->",initialRows)
    ...
    console.log("Grid Data Updated:", initialRows);
  }
};


console.log("gridDtaaaaaaaaaaaaaaaaaa",gridData)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (currentSavedData.voucherNumber || data.voucherNumber) {
        ...
        else console.error('voucherDate is null. Cannot call getVoucherData.');
        ...
        if(response.voucherNumber){
          console.log("KKKKKKKmmm------inside response---------->",popupState)
          setPopupState({
            ...popupState,
            isAlertOpen: true,
  ...
  const handleInputChange = async ({ rowIndex, header, value }: any) => {
    const newGridData = [...gridData];
    console.log("gridData---1-->",gridData, newGridData)
