...
module.exports.addParty = async (partyData) => {
  console.log("ledger Party data ====> ", partyData);
  ...
  try {
    ...
    const groupData = (await groups.getAll("", "", "", "", ""))
    ...
    if (partyData.general_info) {
      ...
      data.forEach((s) => {
        if (partyData.general_info.stationName === s.group_name) {
      ...
      // for balance-info
      if (partyData.balance_contact_info && partyData.balance_contact_info.balance_info) {
        partyData.balance_contact_info.balance_info.party_id = PartyCode+1;
        partyData.balance_contact_info.balance_info.openingBalType = partyData.balance_contact_info.balance_info.openingBalType === 'DR' ? 'debit' : 'credit';
        await balance.insert(partyData.balance_contact_info.balance_info);
      }

      // for contacts 

      if((partyData.balance_contact_info && partyData.balance_contact_info.contacts_info) || (partyData.tax_personal_details && partyData.tax_personal_details.personal_info)){
        const data = {
          ...
          maritalStatus: partyData.tax_personal_details.personal_info.maritalStatus,
          designation: partyData.tax_personal_details.personal_info.designation,
          website_input: partyData.tax_personal_details.personal_info.website_input,
      ...
      // for licence part 
      if(partyData.tax_personal_details && partyData.tax_personal_details.licence_info){
      ...
      // for gst Part
      if(partyData.tax_personal_details && partyData.tax_personal_details.gst_data){
        ...
        if(partyData.tax_personal_details && partyData.tax_personal_details.bank_details){
          partyData.tax_personal_details.bank_details.party_id = PartyCode+1;
          await accountDetails.insert(partyData.tax_personal_details.bank_details);
        }
